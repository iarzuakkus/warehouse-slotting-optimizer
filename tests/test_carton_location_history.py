...
def create_movement_context(
    ...
    source_id = create_location(db_client, f"{suffix}-SOURCE", "01")
    target_id = create_location(db_client, f"{suffix}-TARGET", "02")
...
def test_carton_movement_lifecycle(db_client: TestClient) -> None:
    carton_id, source_id, target_id = create_movement_context(db_client, "001")
...
def test_rejects_movement_to_same_location(db_client: TestClient) -> None:
    carton_id, source_id, _ = create_movement_context(db_client, "002")
...
def test_rejects_movement_to_inactive_location(db_client: TestClient) -> None:
    carton_id, _, target_id = create_movement_context(db_client, "003")
...
def test_rejects_missing_target_location(db_client: TestClient) -> None:
    carton_id, _, _ = create_movement_context(db_client, "004")
...
def test_can_remove_carton_from_location(db_client: TestClient) -> None:
    carton_id, source_id, _ = create_movement_context(db_client, "005")
